...
psrc_sound_struct psrc_sound;

void* psrc_sound_smThread(void* data) {
    (void)data;
    while (Mix_FadingMusic() != MIX_NO_FADING) {SDL_Delay(25);}
    if (!Mix_FadeOutMusic(psrc_sound.music_fadeOut)) {psrc.displayError(PSRC_ERR, "Mix_FadeOutMusic", (char*)Mix_GetError());}
...
void* psrc_sound_mpThread(void* data) {
    ...
    if (Mix_FadeInMusic(psrc_sound_music, -1, psrc_sound.music_fadeIn)) {psrc.displayError(PSRC_ERR, "Mix_FadeInMusic", (char*)Mix_GetError()); return NULL;}
    return NULL;
}
...
bool psrc_sound_waitPlayMusic(char* filename) {
    ...
    if (Mix_FadeInMusic(psrc_sound_music, -1, psrc_sound.music_fadeIn)) {psrc.displayError(PSRC_ERR, "Mix_FadeInMusic", (char*)Mix_GetError()); return false;}
    return true;
}
...
psrc_sound_struct* psrc_sound_init() {
    ...
    psrc_sound = (psrc_sound_struct){3000, 3000, psrc_sound_playMusic, psrc_sound_waitPlayMusic, psrc_sound_stopMusic, psrc_sound_waitStopMusic, psrc_sound_deinit};
    return &psrc_sound;
}
